...
export function FinalReward({ onReset, onBackToHome }) {
  ...
  useEffect(() => {
    if (phase === "gather") {
      const interval = setInterval(() => {
        setActiveTreasureIndex((prev) => (prev + 1) % TREASURES.length);
      }, 2000);
  ...
  return (
    <div className="relative w-full h-full min-h-screen overflow-hidden bg-black text-white flex flex-col items-center justify-center p-4">
      ...
      {/* BACKGROUND PHASES */}
      {/* Phase 1-3: Dark Cave background */}
      ...
      {/* Phase 4: Revealing Transition Flash */}
      {phase === "revealing" && (
        <div className="absolute inset-0 z-40 bg-white flex items-center justify-center animate-[flash-in_1.8s_ease-out_forwards]">
          <div className="w-full h-full bg-[radial-gradient(circle,rgba(255,236,179,1)_0%,rgba(255,255,255,1)_60%,rgba(245,158,11,1)_100%)]" />
        </div>
      )}

      {/* Phase 5: Real World revealed! */}
      {phase === "victory" && (
        <div 
      ...
      {/* Ambient cave dark/shadow overlay for non-victory phases */}
      ...
      {/* Victory ambient background glow */}
      {phase === "victory" && (
      ...
      {/* MAIN LAYOUT */}
      <div className="relative z-10 w-full max-w-[1100px] flex flex-col items-center justify-between min-h-[85vh] py-6">
        
        {/* HEADER / NARRATION PANEL */}
        <div className="text-center max-w-[700px] px-4 space-y-3">
          ...
          <h1 className="font-title text-3xl md:text-5xl font-black tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-yellow-400 to-orange-500 py-2 leading-normal">
          ...
          <p className="text-sm md:text-base leading-relaxed text-amber-100/85">
        ...
        <div className="relative flex items-center justify-center w-full min-h-[380px] md:min-h-[440px] my-4">
          ...
          {phase === "victory" && (
            <div className="relative flex flex-col items-center w-full max-w-[800px] px-2">
              
              {/* THE CERTIFICATE VIEW */}
              {showCertificate ? (
                <div 
                  id="print-area"
                  className="w-full max-w-[680px] h-[450px] rounded-3xl border-[6px] double border-amber-600/70 bg-[#fffdf6] p-6 text-[#5b401e] flex flex-col justify-between shadow-2xl relative overflow-hidden"
                  style={{
                ...
                >
                  ...
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 opacity-[0.03] pointer-events-none">
                    <Sun className="w-80 h-80 text-amber-900" />
                  </div>

                  {/* Top Header */}
                  <div className="text-center mt-2 relative">
                    <Award className="h-8 w-8 text-amber-600 mx-auto mb-1 drop-shadow-sm" />
                    <p className="text-[10px] font-extrabold tracking-[0.24em] text-amber-700">
                      CỘNG HÒA TRI THỨC PHIÊN BẢN CỔ ĐẠI
                    </p>
                    <h2 className="font-title text-2xl md:text-3xl font-black text-amber-900 leading-tight tracking-wide">
                      CHỨNG NHẬN KHAI PHÓNG TÂM TRÍ
                    </h2>
                  </div>

                  {/* Body Text */}
                  <div className="text-center px-4 md:px-8 space-y-4 my-2 relative">
                    <p className="text-xs italic text-amber-700">Chứng nhận này trân trọng vinh danh học viên:</p>
                    
                    <h3 className="font-title text-2xl md:text-3xl font-black text-amber-950 border-b border-dashed border-amber-900/35 pb-1 max-w-[420px] mx-auto min-h-[38px] drop-shadow-sm uppercase">
                      {studentName.trim() || "(Vui lòng điền tên của bạn)"}
                    </h3>

                    <p className="text-[13px] md:text-sm leading-relaxed text-amber-900/90 font-medium">
                  ...
                  <div className="flex justify-between items-end px-4 md:px-8 mb-2 relative">
                    ...
                    {/* Glowing Wax Seal */}
                    <div className="flex justify-center items-center w-16 h-16 rounded-full bg-gradient-to-tr from-[#da251d] to-[#ff5d55] border-2 border-amber-300 shadow-[0_4px_12px_rgba(218,37,29,0.4)] relative">
                      <div className="absolute inset-[3px] rounded-full border border-dashed border-white/20" />
                      <Sun className="h-7 w-7 text-amber-200" />
                      <span className="absolute -bottom-6 text-[8px] font-bold text-amber-800 tracking-widest whitespace-nowrap">ẤN TÍN CHÂN LÝ</span>
                    </div>

                    <div className="text-right w-1/3">
                      <p className="text-[10px] font-extrabold text-amber-800">NGƯỜI DẪN ĐƯỜNG</p>
                      <p className="text-lg font-bold font-serif text-amber-950 mt-1 italic tracking-wider leading-none select-none">
                        Plato
                      </p>
                      <div className="w-16 h-0.5 bg-amber-900/30 ml-auto mt-1" />
                    </div>
                  </div>
                ...
                <Card className="w-full rounded-[38px] border-[#ecc67e] bg-[linear-gradient(135deg,rgba(255,253,248,0.985)_0%,rgba(252,244,222,0.97)_52%,rgba(247,233,198,0.95)_100%)] shadow-[0_28px_90px_rgba(151,118,43,0.3)] text-[#5c411c] overflow-hidden">
                  <CardContent className="p-6 md:p-8 text-center flex flex-col items-center space-y-4">
                    {/* Shiny Emblem */}
                    <div className="relative flex items-center justify-center w-20 h-20 rounded-full bg-gradient-to-tr from-amber-400 to-yellow-600 shadow-[0_8px_30px_rgba(214,158,46,0.35)] border border-[#ebd095]">
                      <Sun className="h-10 w-10 text-white animate-spin-slow" style={{ animationDuration: "20s" }} />
                      <Sparkles className="absolute h-5 w-5 text-amber-200 top-1 right-1 animate-pulse" />
                    </div>
                     ...
                     <div className="space-y-1">
                      ...
                      <h2 className="font-title text-2xl md:text-3xl font-black text-[#5c4017]">
                        BƯỚC RA KHỎI HANG ĐỘNG
                      </h2>
                    </div>

                    <p className="text-sm md:text-[15px] leading-relaxed text-[#755c3c] font-semibold max-w-[500px]">
                    ...
                    <div className="w-full max-w-sm bg-amber-900/5 p-3 rounded-2xl border border-amber-900/10 space-y-2 mt-2">
                      <label className="block text-xs font-bold text-amber-800 text-left">Nhập tên của bạn để cấp Chứng nhận Khai phóng:</label>
                      <div className="flex gap-2">
                        <input 
                          ...
                          className="flex-1 h-9 px-3 rounded-xl border border-amber-300 bg-white text-[#5c411c] font-bold text-xs focus:outline-none focus:ring-1 focus:ring-amber-500"
                        />
                        <Button 
                          onClick={() => studentName.trim() && setShowCertificate(true)}
                          disabled={!studentName.trim()}
                          className="h-9 rounded-xl bg-gradient-to-r from-amber-600 to-amber-700 text-white font-bold text-xs hover:brightness-105"
                        >
                          Nhận chứng nhận
                        </Button>
                      </div>
                    ...
                    <div className="grid grid-cols-6 gap-2 w-full max-w-[420px] pt-3 border-t border-[#eadbb7]">
                      {TREASURES.map((item) => {
                        const IconComponent = item.icon;
                        return (
                          <div key={item.id} className="flex flex-col items-center space-y-1">
                            <div className={`p-2 rounded-full bg-gradient-to-br ${item.color} text-white shadow-sm`} title={item.name}>
        ...
        <div className="relative z-20 flex flex-col sm:flex-row gap-3.5 items-center w-full justify-center px-4">
          {phase === "gather" && (
            <Button
              ...
              {showCertificate ? (
                ...
                <>
                  <Button
                    ...
                    className="h-12 min-w-[160px] rounded-full border-[#d8c49b] bg-white/90 text-[14px] font-bold text-[#5f4621] shadow-lg hover:bg-white transition-all transform hover:scale-[1.02]"
                  >
                  ...
                  <Button
                    onClick={onReset}
                    className="h-12 min-w-[160px] rounded-full bg-[linear-gradient(180deg,#ffe59b,#f5cc63)] border border-[#e5ba51] text-[14px] font-black text-[#4f3718] shadow-lg hover:brightness-105 transition-all transform hover:scale-[1.02]"
                  >
